 ...
 */
class DefaultTokenBuilder(private val algorithm: Algorithm, private val issuer: String) :
    TokenBuilder {
    ...
    override fun buildAccessToken(username: String, expirationMs: Long, authorities: Array<String>): String {
        return buildBaseToken(username, expirationMs)
            .withArrayClaim(net.plshark.usererror.authentication.token.UserErrorClaim.AUTHORITIES, authorities)
            .sign(algorithm)
    }

    override fun buildRefreshToken(username: String, expirationMs: Long): String {
        return buildBaseToken(username, expirationMs)
            .withClaim(net.plshark.usererror.authentication.token.UserErrorClaim.REFRESH, true)
            .sign(algorithm)
    }
