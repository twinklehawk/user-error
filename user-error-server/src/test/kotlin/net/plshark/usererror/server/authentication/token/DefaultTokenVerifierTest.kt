...
class DefaultTokenVerifierTest {
    ...
    fun `valid access tokens should return the username and authorities`() {
        ...
        assertEquals(
            UserAuthorities(username = "test-user", authorities = setOf("user")), verifier.verifyToken(token)
        )
    }
    ...
    fun `no authorities claim should build an empty authorities list`() {
        ...
        assertEquals(UserAuthorities(username = "test-user", authorities = setOf()), verifier.verifyToken(token))
    }
